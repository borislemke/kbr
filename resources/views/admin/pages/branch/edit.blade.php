...
{!! Form::open(['url' => route('api.branch.update', $branch->id)]) !!}

    {{ method_field('PUT') }}
    <div class="m-input-group fwidth flexbox justify-between">
        <div class="m-input-wrapper w50-6">
            <input value="{{ $branch->name }}" type="text" name="name" required>
            <label for="title">name</label>
        </div>
        <div class="m-input-wrapper w50-6">
            <input value="{{ $branch->manager }}" type="text" name="manager" required>
    ...
    <div class="m-input-group fwidth flexbox justify-between">
        <div class="m-input-wrapper w50-6">
            ...
            <select name="city">
                ...
                <option value="{{ $city->city_name }}" {{ $branch->city == $city->city_name ? 'selected' : '' }}>{{ $city->city_name }}</option>
                @endforeach
...
<script>
    ...
    $(document).ready(function() {
        ...
        $(document).on('click', '[save-form]', function(event) {
            ...
            var url = "{{ route('api.branch.update', $branch->id) }}";
